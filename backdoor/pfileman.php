   ...
   <head>
	...
	<form action="" method="POST" name="input" ENCTYPE="multipart/form-data">
		...
		<?php
			...
			echo "server:".date_default_timezone_get()."<br><br>";
			date_default_timezone_set("Asia/Jakarta");
			echo "client:".date_default_timezone_get()."<br><br>";
...
<?php
			...
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					if (is_file($path.$file)) {
						$link = "<a href='/payload.php?target=$file'><h4>$file <br>---->> ".date("d-m-Y H:i:s", fileatime($path.$file)).'<br>---->> '.size(filesize($path.$file))."</h4></a>";		
	...
	// download
		...
		$dir_downlad = dirname(_FILE__)."/listclient/";
